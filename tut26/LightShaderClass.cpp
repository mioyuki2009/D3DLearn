...
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture1, XMFLOAT4 clipPlane, float alpha)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture1, clipPlane, alpha);
	if (!result)
	{
		return false;
	}
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture1, XMFLOAT4 clipPlane, float alpha)
{
	...
	Effects::BasicFX->SetAlpha(alpha);
	return true;
}
