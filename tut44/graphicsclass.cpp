...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_GroundModel->Initialize(m_D3D->GetDevice(), "floor.txt", "stone.dds");
	if (!result)
	{
	...
	result = m_CubeModel->Initialize(m_D3D->GetDevice(), "cube.txt", "seafloor.dds");
	if (!result)
	{
	...
	result = m_ProjectionTexture->Initialize(m_D3D->GetDevice(), "grate.dds");
	if (!result)
	{
...
bool GraphicsClass::Render()
{
	...
	worldMatrix = worldMatrix * XMMatrixTranslation(0.0f, 1.0f, 1.0f);
