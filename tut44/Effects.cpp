	...
	: Effect(device, filename)
{
	...
	ViewMatrix2 = mFX->GetVariableByName("viewMatrix2")->AsMatrix();
	ProjectionMatrix2 = mFX->GetVariableByName("projectionMatrix2")->AsMatrix();

	Texture = mFX->GetVariableByName("shaderTexture")->AsShaderResource();
	ProjectionTexture = mFX->GetVariableByName("projectionTexture")->AsShaderResource();
