	...
	ID3D11ShaderResourceView* depthMapTexture2, XMFLOAT3 lightPosition2, XMFLOAT4 diffuseColor2)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightViewMatrix, lightProjectionMatrix,
		texture, depthMapTexture, lightPosition, ambientColor, diffuseColor, lightViewMatrix2, lightProjectionMatrix2, 
		depthMapTexture2, lightPosition2, diffuseColor2);
	if (!result)
	{
		return false;
	}
	...
	XMFLOAT4 diffuseColor2)
{
	...
	Effects::BasicFX->SetLightViewMatrix2(lightViewMatrix2);
	Effects::BasicFX->SetLightProjectionMatrix2(lightProjectionMatrix2);
	Effects::BasicFX->SetDepthMapTexture2(depthMapTexture2);

	Effects::BasicFX->SetLightPosition2(lightPosition2);
	auto vecDiffuseColor2 = XMLoadFloat4(&diffuseColor2);
	Effects::BasicFX->SetDiffuseColor2(vecDiffuseColor2);
