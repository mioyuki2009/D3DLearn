...
bool TextureClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* context, const CHAR* filename1, const CHAR* filename2, const CHAR* filename3)
{
	// Load the texture in.
	int size = 2;
	...
	HR(device->CreateShaderResourceView(texArray, &viewDesc, &m_textureArray));
	//
	...
	HR(D3DX11CreateShaderResourceViewFromFile(device, filename3, NULL, NULL, &m_texture, NULL));
	return true;
}
