...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_FloorModel->Initialize(m_D3D->GetDevice(), "floor.txt", "grid01.dds");
	if (!result)
	{
	...
	result = m_BillboardModel->Initialize(m_D3D->GetDevice(), "square.txt", "seafloor.dds");
	if (!result)
	{
		MessageBox(hwnd, "Could not initialize the billboard model object.", "Error", MB_OK);
		return false;
	}
