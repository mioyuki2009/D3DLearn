...
bool HorizontalBlurShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd)
{
	HRESULT result;
	...
	Effects::BasicFX->PassTech4->GetPassByIndex(0)->GetDesc(&passDesc);
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenWidth)
{
	...
	Effects::BasicFX->SetScreenWidth(screenWidth);
	return true;
}
...
void HorizontalBlurShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	Effects::BasicFX->PassTech4->GetPassByIndex(0)->Apply(0, deviceContext);
	// Render the triangle.
