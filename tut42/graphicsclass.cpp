...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_CubeModel->Initialize(m_D3D->GetDevice(), "cube.txt", "wall01.dds");
	if (!result)
	{
	...
	result = m_SphereModel->Initialize(m_D3D->GetDevice(), "sphere.txt", "ice.dds");
	if (!result)
	{
	...
	result = m_GroundModel->Initialize(m_D3D->GetDevice(), "plane01.txt", "metal001.dds");
	if (!result)
	{
	...
	if (!result)
	{
		MessageBox(hwnd, "Could not initialize the black and white render to texture object.", "Error", MB_OK);
		return false;
	}
	...
	if (!result)
	{
		MessageBox(hwnd, "Could not initialize the soft shadow shader object.", "Error", MB_OK);
		return false;
	}
...
bool GraphicsClass::RenderSceneToTexture()
{
	...
	worldMatrix = worldMatrix * XMMatrixTranslation(posX, posY, posZ);
