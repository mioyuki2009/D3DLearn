...
struct VertexInputTypeFade
{
	float4 position : POSITION;
	float2 tex : TEXCOORD0;
};

struct PixelInputTypeFade
{
	float4 position : SV_POSITION;
	float2 tex : TEXCOORD0;
};
...
PixelInputTypeFade FadeVertexShader(VertexInputTypeFade input)
{
	PixelInputTypeFade output;
...
float4 FadePixelShader(PixelInputTypeFade input) : SV_TARGET
{
	float4 color;
...
technique11 Pass2
{
	pass P0
	{
		SetVertexShader(CompileShader(vs_5_0, FadeVertexShader()));
		SetGeometryShader(NULL);
		SetPixelShader(CompileShader(ps_5_0, FadePixelShader()));
	}
}